class Fraccion:
    def __init__(self,numerador,denominador):
        self.num=numerador
        self.den=denominador

    def __mul__(self,other):
        r=Fraccion(0,0)
        r.num=self.num*other.num
        r.den=self.den*other.den
        return r

    def __repr__(self):
        entero=self.num//self.den
        if entero>0:
            resto=self.num%self.den
            if resto>0:
                return "{0} {1}/{2}".format(entero,resto,self.den)
            else:
                return "{0}".format(entero)
        else:
            return "{0}/{1}".format(self.num,self.den)

    def __add__(self,other):
        s=Fraccion(0,0)
        if self.den!=other.den:
            if other.den%self.den!=0 or self.den%other.den!=0:
                s.den=self.den*other.den
                f1=other.den*self.num
                f2=self.den*other.num
                s.num=f1+f2
            elif other.den%self.den==0 or self.den%other.den==0:
                if other.den>self.den:
                    s.den=other.den
                    m=other.den/self.den
                    q1=m*self.den
                    s.num=q1+other.num
                if self.den>other.den:
                    s.den=self.den
                    n=self.den/other.den
                    q2=n*other.den
                    s.num=self.num+q2
        else:
            s.num=self.num+other.num
            s.den=self.den
        return s
    def __sub__(self,other):
        res=Fraccion(0,0)
        if self.den!=other.den:
            if other.den%self.den!=0 or self.den%other.den!=0:
                res.den=self.den*other.den
                f1=other.den*self.num
                f2=self.den*other.num
                res.num=f1-f2
            elif other.den%self.den==0 or self.den%other.den==0:
                if other.den>self.den:
                    res.den=other.den
                    m=other.den/self.den
                    q1=m*self.den
                    res.num=q1-other.num
                if self.den>other.den:
                    res.den=self.den
                    n=self.den/other.den
                    q2=res*other.den
                    s.num=self.numresq2
        else:
            res.num=self.num+other.num
            res.den=self.den
        return res

    def __truediv__(self,other):
        d=Fraccion(0,0)
        d.num=self.num*other.den
        d.den=self.den*other.num
        return d
    def a_numero(self):
        return self.num/self.den
if __name__ == "__main__":
    f=input("Ingresa la primera fraccion [a/b]: ")
    f=f.split("/")
    f1=Fraccion(int(f[0]),int(f[1]))
    f=input("Ingresa la segunda fraccion [c/d]: ")
    f=f.split("/")
    f2=Fraccion(int(f[0]),int(f[1]))
    print("La suma es ",f1+f2)
    print("La resta es ",f1-f2)
    print("La multiplicación es ",f1*f2)
    print("La división es ",f1/f2)
         