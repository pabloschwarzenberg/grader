...
class Carro:
    ...
    def checkout(self,tipoDespacho,direccion):
        print(" 0: despacho aereo normal")
        print(" 1: despacho aereo express")
        monto=0
        for item in self.lista_de_productos:#recorer carro 
            monto+=item.cantidad * item.producto.precio # cantidad por precio
        print("monto compra: ",monto)

        if tipoDespacho==0 and direccion=="usa":
            self.descuento=20
            self.total=((((monto*(100-self.descuento))/100)*120)/100)

        if tipoDespacho==1 and direccion=="usa":
            self.descuento=20
            self.total=((((monto*(100-self.descuento))/100)*130)/100)

        if tipoDespacho==0 and "chile" in direccion:
            self.descuento=20
            self.total=((((((monto*(100-self.descuento))/100)*120)/100)*120)/100)

        if tipoDespacho==1 and "chile" in direccion:
            self.descuento=20
            self.total=((((((monto*(100-self.descuento))/100)*130)/100)*120)/100)
