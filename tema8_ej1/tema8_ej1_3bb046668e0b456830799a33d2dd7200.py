...
class Carro:
    ...
    def calcular_descuentos(self):
        self.descuento = 0
        for item in self.lista_de_productos:
            self.descuento += (item.producto.precio * item.cantidad * 0.1)  # 10% de descuento

    def calcular_valor_despacho(self, tipoDespacho):
        if tipoDespacho == 0:
            return sum(item.producto.precio * item.cantidad for item in self.lista_de_productos) * 0.2  # 20% del valor de los productos
        elif tipoDespacho == 1:
            return sum(item.producto.precio * item.cantidad for item in self.lista_de_productos) * 0.3  # 30% del valor de los productos
        else:
            return 0
    ...
    def checkout(self, tipoDespacho, direccion):
        ...
        self.total = self.total - self.descuento + valor_despacho + impuesto
        return self.total
