    ...
    def checkout(self,tipoDespacho,direccion):
        for item in self.lista_de_productos:
            if item.producto.stock >= 5:
                self.descuento += item.producto.precio * 0.1 #descuento del 10%
            elif item.producto.stock >= 3:
                self.descuento += item.producto.precio * 0.05 #descuento del 5%
            
    #Calculando valor del despacho
        valor_despacho = 0
        if tipoDespacho==0: #Despacho Aéreo Normal
            valor_despacho = sum([item.producto.precio * item.cantidad for item in self.lista_de_productos]) * 0.2
        elif tipoDespacho==1: #Despacho Aéreo Express
            valor_despacho = sum([item.producto.precio * item.cantidad for item in self.lista_de_productos]) * 0.3
        
    #Calculando valor total con impuesto
        if "chile" in direccion.lower():
            self.total = sum([item.producto.precio * item.cantidad for item in self.lista_de_productos]) - self.descuento + valor_despacho + (sum([item.producto.precio * item.cantidad for item in self.lista_de_productos]) - self.descuento + valor_despacho) * 0.2
        else:
            self.total = sum([item.producto.precio * item.cantidad for item in self.lista_de_productos]) - self.descuento + valor_despacho
