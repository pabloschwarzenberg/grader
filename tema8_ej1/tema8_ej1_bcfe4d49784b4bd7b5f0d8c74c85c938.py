...
class Carro:
    ...
    def calcular_descuentos(self):
        descuentos = sum(item.producto.precio * 0.1 * item.cantidad for item in self.lista_de_productos)
        return descuentos
    ...
    def checkout(self, tipoDespacho, direccion):
        ...
        valor_total = sum(item.producto.precio * item.cantidad for item in self.lista_de_productos) - descuentos
        if tipoDespacho == 0:  # Despacho Aéreo Normal
