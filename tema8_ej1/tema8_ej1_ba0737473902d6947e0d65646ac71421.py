...
class Carro:
    ...
    def checkout(self,tipoDespacho,direccion):
        ...
        for i in range (len(self.lista_de_productos)):
            total=total+(self.lista_de_productos[i].producto.precio)*self.lista_de_productos[i].cantidad
        g=[]
        for i in range(len(self.lista_de_productos)):
            g.append( str(self.lista_de_productos[i].producto.codigo))
                ...
                for t in range(len(g)):
                    if g[t]=="2":
                        for k in range(len(g)):
                            if g[k]=="3":
                                self.descuento=total*0.2
        total=total-self.descuento
        if int(tipoDespacho)==0:
            total=total*1.2
        elif int(tipoDespacho)==1:
            total=total*1.3
        if direccion.upper().find("CHILE")!=-1:
            total= total*1.2
        self.total=total
