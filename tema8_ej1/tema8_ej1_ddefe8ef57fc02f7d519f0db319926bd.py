...
class Carro:
    ...
    def checkout(self,tipoDespacho,direccion):
        ...
        for i in self.lista_de_productos:
            t += i.cantidad * i.producto.precio
            pc.append(i.producto.codigo)
        desc = 0
        if 1 in pc and 2 in pc and 3 in pc:
            desc += t * 20 / 100
        if 4 in pc and 5 in pc:
            desc += t * 15 / 100
        pt = t - desc
        if tipoDespacho == 0:
            cd = pt * 20 / 100
        ...
        ptt = pt + cd
        if i != 0:
            return(float(round(ptt + ptt * 20 / 100)))
        else:
            return(float(round(ptt)))
