...
def checkout(self, tipoDespacho, direccion):
    ...
    for item in self.items:
        descuentos += item.producto.precio * item.cantidad

    # Calcular el valor del despacho
    valorDespacho = 0
    if tipoDespacho == 0:  # Despacho Aéreo Normal
        valorDespacho = 0.2 * descuentos
    elif tipoDespacho == 1:  # Despacho Aéreo Express
        valorDespacho = 0.3 * descuentos

    # Calcular el valor total a pagar
    valorTotal = descuentos - valorDespacho
    if "chile" in direccion.lower():  # Si el despacho es hacia Chile, agregar el 20% de impuesto
        valorTotal *= 1.2
