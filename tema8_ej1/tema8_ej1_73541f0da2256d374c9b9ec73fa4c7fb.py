...
class Carro:
    ...
    def calcular_descuentos(self):
        self.descuento = 0
        for item in self.lista_de_productos:
            descuento_producto = item.producto.precio * 0.1
            self.descuento += descuento_producto * item.cantidad

    def calcular_despacho(self, tipoDespacho):
        if tipoDespacho == 0:
            self.despacho = self.total * 0.2
        elif tipoDespacho == 1:
            self.despacho = self.total * 0.3
...
if __name__ == "__main__":
    ...
    while True:
        ...
        item_ingresado = input(
            "Ingresa un producto a comprar (numero,cantidad): ")
        if item_ingresado == "":
            break
        else:
