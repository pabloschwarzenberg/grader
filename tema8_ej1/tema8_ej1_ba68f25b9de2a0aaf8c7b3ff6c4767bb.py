...
class Carro:
    ...
    def checkout(self, tipoDespacho, direccion):
        ...
        if tipoDespacho == 0:
            despacho = 0.2 * self.total
        elif tipoDespacho == 1:
            despacho = 0.3 * self.total
        else:
            despacho = 0

        self.total -= self.descuento  # Restar descuentos
        self.total += despacho  # Agregar valor del despacho
    ...
    def __repr__(self):
        s = "Carro de Compras:\n"
        for item in self.lista_de_productos:
            s += "{item.cantidad}: {item.producto.nombre}\n"
        return s
