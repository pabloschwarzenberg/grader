...
class Carro:
    ...
    def checkout(self,tipoDespacho,direccion):
        self.total=0
        for item in self.lista_de_productos:
            self.total+=int(item.producto.precio*item.cantidad*0.8)
        if tipoDespacho==0:
            self.total=int(self.total*1.2)
        elif tipoDespacho==1:
            self.total=int(self.total*1.3)
        direccion=direccion.split(" ")
        if direccion[-1]=="Chile":
            return int(self.total*1.2)
        else:
