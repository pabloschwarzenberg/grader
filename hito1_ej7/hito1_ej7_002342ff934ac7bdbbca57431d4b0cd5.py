#Zodiaco
dia=int(input("ingrese su dia de nacimiento:"))
mes=int(input("ingrese mes de nacimiento(numero):"))

if ((mes==3) and (dia>=21)) or ((mes==4) and (dia<=19)):n=int(input("ingrese in numero de hasta 4 dijitos:"))

if ((mes==4) and (dia>=20)) or ((mes==5) and (dia<=20)):
 print("tu signo zodiacal es tauro")

if ((mes==5) and (dia>=21)) or ((mes==6) and (dia<=20)):
 print("tu signo zodiacal es geminis")

if ((mes==6) and (dia>=21)) or ((mes==7) and (dia<=22)):
 print("tu signo zodiacal es cancer")

if ((mes==7) and (dia>=23)) or ((mes==8) and (dia<=22)):
 print("tu signo zodiacal es leo")

if ((mes==8) and (dia>=23)) or ((mes==9) and (dia<=22)):
 print("tu signo zodiacal es virgo")

if ((mes==9) and (dia>=23)) or ((mes==10) and (dia<=22)):
 print("tu signo zodiacal es libra")

if ((mes==10) and (dia>=23)) or ((mes==11) and (dia<=21)):
 print("tu signo zodiacal es escorpion")

if ((mes==11) and (dia>=22)) or ((mes==12) and (dia<=21)):
 print("tu signo zodiacal es sagitaro")

if ((mes==12) and (dia>=22)) or ((mes==1) and (dia<=19)):
 print("tu signo zodiacal es capricornio")

if ((mes==1) and (dia>=20)) or ((mes==2) and (dia<=18)):
 print("tu signo zodiacal es acuario")

if ((mes==2) and (dia>=19)) or ((mes==3) and (dia<=20)):
 print("tu signo zodiacal es piscis")
