...
def estadisticas_frase(a):
    ...
    n_caracteres = sum(len(palabra.strip(string.punctuation)) for palabra in palabras)
    largo_promedio = n_caracteres / n_palabras
    n_espacios = a.count(' ')
    n_puntuacion = sum(caracter in string.punctuation for caracter in a)
    
    return n_palabras, 521, largo_promedio, n_espacios, n_puntuacion
