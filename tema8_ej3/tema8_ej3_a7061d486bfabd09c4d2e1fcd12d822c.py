...
def estadisticas_frase(frase):
    ...
    num_caracteres = sum(len(palabra) for palabra in palabras if palabra.strip(string.punctuation))
    largo_promedio = num_caracteres / num_palabras
    num_espacios = frase.count(' ')
    num_puntuacion = sum(1 for caracter in frase if caracter in string.punctuation)
