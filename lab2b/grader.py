...
class Analizador(analizador.util.AnalizadorBase):
    def __init__(self):
        ...
        for i in range(4):
            ...
            self.output.append([self.divisores_alternados(inicio,fin,x,y)])
        self.setup.append([str(-100),str(-50)])
        self.output.append([self.divisores_alternados(-100,-50,x,y)])
    ...
    def divisores_alternados(self,inicio,fin,x,y):
        i=inicio
        s=""
        while i<=fin:
            if i%x==0 or i%y==0:
