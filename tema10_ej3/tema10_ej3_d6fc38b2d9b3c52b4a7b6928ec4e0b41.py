def buscar_palabra(tablero, palabra, fila, columna, direccion):
    ...
    if direccion == "derecha":
        ...
        # Verificar si la palabra cabe en la dirección abajo
        ...
        # Verificar si la palabra cabe en la dirección diagonal
    ...
    for i in range(len(tablero)):
        for j in range(len(tablero[i])):
            if buscar_palabra(tablero, palabra, i, j, "derecha"):
                posiciones.append((palabra, [i, j], "derecha"))
            if buscar_palabra(tablero, palabra, i, j, "abajo"):
                posiciones.append((palabra, [i, j], "abajo"))
            if buscar_palabra(tablero, palabra, i, j, "diagonal"):
                posiciones.append((palabra, [i, j], "diagonal"))
    return posiciones
...
def sopaletras(nombre_archivo, palabras):
    ...
    with open(nombre_archivo, "r") as f:
        tablero = [list(linea.strip()) for linea in f]
    resultados = []
    for palabra in palabras:
        posiciones = buscar_en_tablero(tablero, palabra)
        if posiciones:
            resultados.append(posiciones[0])
    return resultados