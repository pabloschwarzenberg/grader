def verificacion(op,i,j,matriz,palabra):
    if len(palabra) == 1: #termina cuando hay una sola letra
        ...
        try:
            if matriz[i][j].upper() == palabra[0].upper():
                nueva = palabra[1:] 
                resp = verificacion(op,i,j,matriz,nueva)
                if resp:
                    return resp
                return False
        except:
...
def sopaletras(nombre_archivo,palabras):
    ...
    #traspasar la sopa de letras a una matriz
        ...
        for i in range(0,len(matriz)):
            for j in range(0, len(matriz[0])):
                if(matriz[i][j].upper() == palabra[0].upper()):
                    ...
                    for x in range(1,4): 
                        verificar=verificacion(x,i,j,matriz,nueva)
                        if (verificar):
