...
def click(evento):
    ...
    if evento.widget["text"]=="":
        ...
        r=tablero.getCeldasVacias()
        if len(r)==0:
            messagebox.showinfo("Juego del Gato", "Tenemos un Empate")
            sys.exit(0)
...
class Matriz:
    ...
    def getCeldaVaciaColumna(self,j):
        r=[]
        for i in range(len(self.celdas)):
            if self.celdas[i][j]==0:
                r.append([i,j])
    ...
    def getCeldaVaciaDiagonalIzquierda(self):
        r=[]
        for i in range(len(self.celdas)):
            if self.celdas[i][i]==0:
                r.append([i,i])
        return r

    def getCeldaVaciaDiagonalDerecha(self):
        r=[]
        for i in range(len(self.celdas)):
            if self.celdas[i][2-i]==0:
                r.append([i,2-i])
        return r
