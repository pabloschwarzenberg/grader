...
#Dar un mensaje de bienvenida, con un breve instructivo del juego. Debe mostrar un mensaje "Presionar
#<ENTER>para continuar" al final del instructivo.

#ARREGLAR  LA INSTANCIA DE IGUALAR
...
class JuegoDelGato:
    ...
    def indicarEstado(self):
        ...
        d = str(self.tablero[0][0])+str(self.tablero[1][0])+str(self.tablero[2][0])
    ...
    def jugarRaton(self):
        ...
        if self.indicarEstado()==0:
            ...
            while self.tablero[a][b]!="   ":
                a = random.randint(0,2)
                b = random.randint(0,2)
            if self.tablero[a][b]=="   ":
                self.tablero[a].pop(b)
                self.tablero[a].insert(b, " O ")
                print("ALEATORIO")
                print(a)
                print(b)
        return True
...
#en esta función la idea es poner el tablero en el estado dado por la matriz
#par que openedx pueda revisar el juego y ver si hace una jugada inteligente
#        matriz = self.tablero[:]
        for i in range(3):
            for j in range(3):
                if matriz[i][j]==0:
