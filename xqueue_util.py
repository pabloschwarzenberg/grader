...
def login(session, url, username, password):
    ...
    (success, msg) = parse_xreply(str(response.content,response.encoding))
    return success, msg
...
def _http_post(session, url, data, timeout):
    '''
        Contact evaluador controller, but fail gently.
...
def post_results_to_xqueue(session, header, body):
    """
        Post the results from a evaluador.db back to xqueue.
        Input:
